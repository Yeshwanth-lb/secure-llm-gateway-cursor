...
**Workspace response-capture — final state (2026-07-30).** Redaction (security) works on ALL 7 surfaces. Response-text capture for the audit log: **gemini.google.com / Docs / Sheets / Slides = reliable** (semantic selectors + text-change detection + last-non-empty; + the generation-wait fix below). **Chat / Gmail / Drive = response `(none)` by design** — these are obfuscated, rotating-class `div[role=listitem]` panels with no stable selector, and the shape-based capture proved a DEAD END: too tight → nothing; wide enough to find the reply (tried in `9314f14`) → it climbed into the message-LIST and logged sender/timestamp metadata ("Ask Gemini , 1 min ,") as the reply. **A wrong pairing is worse than a blank one**, so `9314f14`'s scope widening was REVERTED (`findScope` back to `MAX_SCOPE_HOPS=8` + tight threshold). Reliable capture on these three needs a dedicated approach (a TOP-frame reply watcher that pairs the user bubble with the assistant `listitem` and rejects chips), NOT shape tweaks — deferred; it is audit-visibility only, not a security gap (Gmail/Drive compose in the cross-origin `chat.google.com` iframe, which `all_frames` DOES redact).
