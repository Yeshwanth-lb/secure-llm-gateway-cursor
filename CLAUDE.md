...
**Workspace response-capture — final state (2026-07-29).** Redaction (security) works on ALL 7 surfaces. Response-text capture for the audit log: **gemini.google.com / Docs / Sheets / Slides = reliable** (semantic selectors + text-change detection + last-non-empty + shape fallback). **Chat = mostly reliable** after widening `findScope` (`9314f14`) so the reply is inside the shape-search scope; occasional first-turn/timing miss remains (reply rendered before the first sample → not counted as "appeared after submit"). **Gmail / Drive = response still `(none)`** — the composer is in the cross-origin `chat.google.com` panel iframe (so `all_frames` gave us redaction there) but the REPLY renders in the TOP frame as an obfuscated rotating-class `div[role=listitem]`; the capturing frame (iframe) can't read the top-frame reply. Closing Gmail/Drive needs a dedicated TOP-frame reply watcher (pair the user bubble with the assistant `listitem`, reject chips) — deferred; it's audit-visibility only, not a security gap. The obfuscated `listitem` panels can never be 100% via shape alone (a short reply and a suggestion chip are indistinguishable bare `listitem`s).
